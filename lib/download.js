...
const tmpDir = path.join(os.tmpdir(), `vscode-zeromq-files-${packageVersion}`);
...
async function getAssetsFromGithubApi(opts) {
  ...
  const assets = jsonRelease.assets.filter(
    (a) =>
      ...
      ((a.name.includes("linux-") || a.name.includes("darwin-")) &&
...
async function getAssetFromGithubApi(opts, asset) {
  ...
  const assetDownloadFile = path.join(tmpDir, folder, targetFile);
