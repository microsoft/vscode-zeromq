...
async function getAssetsFromGithubApi(opts) {
  ...
  const assets = jsonRelease.assets.filter(
    (a) =>
      (a.name.endsWith(".node") && a.name.includes("node.napi.")) ||
      a.name.endsWith(".dll")
  );
  if (!assets.length) {
...
async function getAssetFromGithubApi(opts, asset) {
  ...
  // If we're downloading the msvcp140.dll, then the files go into x64 (not too concerned about the 32bit for now).
  const arch = asset.name.endsWith(".dll") ? "x64" : archs.join("+");
  // If we're downloading the win32_x64_msvcp140.dll, then the name is msvcp140.dll
  const targetFile = asset.name.endsWith(".dll")
    ? asset.name
    : `node.napi.${libc}.node`;
...
<<<<<<< HEAD
  console.log(
    `Downloading asset ${asset.name} from ${asset.url} into ${assetDownloadPath}`
  );
=======
  console.log(`Downloading asset ${asset.name} from ${asset.url} into ${assetDownloadPath}`);
>>>>>>> 2518e67 (Add support for downloading windows dlls)
 ...
 * @param {{ 'win32'?: ('x64' | 'ia32' | 'arm64')[]; 'linux'?: ('x64' | 'arm64' | 'armhf')[]; 'darwin'?: ('x64' | 'arm64')[]; 'alpine'?: ('x64' | 'arm64')[]; }} platformOptions If not provided, then downloads all binaries for all platforms and archs.
